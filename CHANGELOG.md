...
- Dunning: configurable attempts / retry interval / grace window (`past_due` keeps `isActive()` true until grace ends); a declined card during a trial never cancels it. A due renewal with no saved card at all now gets the same grace/retry treatment as a declined charge (`Subscription::recordRenewalFailure()`) instead of stalling silently in `active`.
