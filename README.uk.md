...
WayForPay — виняток, і не з нашої волі: його serviceUrl-колбек взагалі не документує поля з сумою повернення (`amount` описаний як «Amount of order», і нічого не сказано, що він містить при реверсі), тож записувати нема чого, крім здогадки. Такі реверси голосно логуються, а не відкидаються: шукай `a reversal was reported for a payment but not recorded` і записуй їх вручну.
