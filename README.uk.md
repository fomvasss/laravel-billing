...
Рядок повернення створюється тільки для грошей, які реально йдуть назад: гейтвей, що відмовив у поверненні, кидає `BillingException` (усі три відповідають на відмову HTTP 200 зі статусом у тілі — `->throw()` такого не спіймає), а `refundedAmount()` рахує і soft-deleted рядки: приховування рядка повернення не має відкривати місце повернути ту саму суму вдруге.
...
Підтримується там, де в гейтвея є refund-API: Monobank, LiqPay, Stripe (`RefundsPayments` — перевіряйте `Billing::gateways()[$name]['capabilities']['refunds']`). Повернення WayForPay/Hutko робляться в кабінеті банку.
