...
| LiqPay | `reversed` | **ні** — пишеться warning |
| WayForPay | `Refunded` / `Voided` | **ні** — пишеться warning |

Останні два розпізнаються й голосно логуються, а не тихо відкидаються, але рядок не створюється: яке поле їхнього колбека несе повернуту суму (і чи це одне повернення, чи накопичувальний підсумок) не звірено з живим акаунтом, а записати неправильну цифру гірше, ніж мати задокументовану прогалину. Шукай у логах `a reversal was reported for a payment but not recorded` і записуй такі повернення вручну, доки це не звірено.
