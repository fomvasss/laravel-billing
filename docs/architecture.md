...
- **`process-recurring-charges`** (every minute, `withoutOverlapping`): pass 1 finalizes due `cancels_at`; pass 2 charges due subscriptions unless a renewal `Payment` is still pending (double-charge guard) or `next_retry_at` is in the future (dunning pacing). Per-subscription try/catch — one broken gateway doesn't strand the batch. It only *initiates*; outcomes come back through the webhook pipeline. The `ChargeOptions` it charges with come from `RenewalChargeOptionsContract` (default: empty, or a generic one-line basket under `billing.renewal.receipt_items`) — resolved inside the same try/catch, so a resolver that throws writes the payment off as failed instead of stranding it pending.
